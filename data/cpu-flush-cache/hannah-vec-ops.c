...
int main(int argc, char **argv)
{
  ...
  ierr = VecNorm(x,NORM_2,&norm1);CHKERRQ(ierr); /* Hannah: clear cache */
  PetscBarrier(NULL);
  ierr = VecDot(w,v,&val);CHKERRQ(ierr); 
  ierr = VecNorm(y,NORM_1,&norm1);CHKERRQ(ierr); /* Hannah: clear cache */
  PetscBarrier(NULL);
  ierr = VecSet(v,0.0);CHKERRQ(ierr);
  ierr = VecScale(x, 42.5);CHKERRQ(ierr);
  ierr = VecNorm(y,NORM_2,&norm2);CHKERRQ(ierr); /* Hannah: clear cache */
  PetscBarrier(NULL);
  ierr = VecCopy(v,w);CHKERRQ(ierr);
  PetscLogStagePop();

  ierr = PetscPrintf(PETSC_COMM_WORLD,"Hannah: test completed successfully!\n");CHKERRQ(ierr);
  ierr = VecDestroy(&v);CHKERRQ(ierr);ierr = VecDestroy(&w);CHKERRQ(ierr);
